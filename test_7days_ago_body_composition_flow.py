...
def capture_7days_ago_body_composition():
    print("==========================================")
    print("  体組成 (7日前: 2026/8/7) 取得フロー開始 ")
    print("==========================================")
    ...
    # 7日前データ行 (X=800, Y=520) をタップ
    print("[Step 2] 7日前データ行 (X=800, Y=520) をタップ...")
    subprocess.run([config.ADB_PATH, "-s", config.DEVICE_ID, "shell", "input", "tap", "800", "520"])
    time.sleep(3)
    
    # 上部スクリーンショット撮影
    print("[Step 3] 7日前 (上部) スクリーンショット撮影...")
    top_png_sd = "/sdcard/body_composition_7days_ago_top.png"
    top_png_local = os.path.join(config.SCREENSHOTS_DIR, "body_composition", "body_composition_7days_ago_top.png")
    ...
    # 下部スクリーンショット撮影
    print("[Step 5] 7日前 (下部) スクリーンショット撮影...")
    bottom_png_sd = "/sdcard/body_composition_7days_ago_bottom.png"
    bottom_png_local = os.path.join(config.SCREENSHOTS_DIR, "body_composition", "body_composition_7days_ago_bottom.png")
    ...
    print("\n==========================================")
    print(" SUCCESS: 7日前 (2026/8/7) の体組成データ取得完了！")
    print("==========================================\n")

if __name__ == "__main__":
    capture_7days_ago_body_composition()
