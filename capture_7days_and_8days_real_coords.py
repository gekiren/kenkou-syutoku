...
def capture_7days_and_8days_real():
    ...
    print("  実測座標: 7日前 Y=410, 8日前 Y=720 (290px × 6日分 = 1740px)")
    print("==========================================")
    ...
    # ==========================================
    # A. 7日前 (2026/8/7) の取得 (実測 Y=410)
    # ==========================================
    print(f"\n--- [7日前: {date_7days_fmt}] データ取得 ---")
    print(f"[Step A-1] 7日前データカード (実測 X=800, Y=410) をタップ...")
    subprocess.run([config.ADB_PATH, "-s", config.DEVICE_ID, "shell", "input", "tap", "800", "410"])
    time.sleep(3)
    ...
    # ==========================================
    # B. 8日前 (2026/8/6) の取得 (実測 Y=720)
    # ==========================================
    print(f"\n--- [8日前: {date_8days_fmt}] データ取得 ---")
    print(f"[Step B-1] 8日前データカード (実測 X=800, Y=720) をタップ...")
    subprocess.run([config.ADB_PATH, "-s", config.DEVICE_ID, "shell", "input", "tap", "800", "720"])
    time.sleep(3)
